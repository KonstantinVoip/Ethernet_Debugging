...
/*все что у нас extern я за комментарил потом разберусь */
...
struct eth_device {
	...
	//пока ставлю заглушку там где bd_t
    int  (*init) (struct eth_device*);
    void (*halt) (struct eth_device*);
    void  (*send) (struct eth_device*,struct eth_device* );
    void  (*recv) (struct eth_device*,struct eth_device*,struct eth_device*);
...
typedef struct {
	u16		et_dest[2];	// Destination node		
	u16		et_src[2];	// Source node			
	u16	    et_type;	// Protocol or length		
} Ethernet_t;
...
typedef struct {
	u16		et_dest[6];	// Destination node		
	u16		et_src[2];	// Source node			
	u16	    et_type;	// Protocol or length		
} Ethernet_t;
...
#define ETHER_HDR_SIZE	14		/* Ethernet header size	14 byte	*/
...
typedef struct {
	...
	u16	  	    total_legth;	// header length and version	
	u16         identefication;
	u16         flags;
...
// extern uchar		NetOurEther[6];		// Our ethernet address	
 extern u16	    	NetOurEther[3];		// Our ethernet address		
 extern uchar		NetServerEther[6];	/* Boot server enet address	*/
 extern IPaddr_t		NetOurIP;		/* Our    IP addr (0 = unknown)	*/
 extern IPaddr_t		NetServerIP;		/* Server IP addr (0 = unknown)	*/
 //extern volatile uchar * NetTxPacket;		// THE transmit packet		
 extern volatile u16 * NetTxPacket;		// THE transmit packet  
...
//extern void NetReceive(volatile u16 *, int);   
  extern u16 NetReceive(volatile u16 *, int);
