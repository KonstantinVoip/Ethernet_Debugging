...
/*
	...
	if (len)
		   ...
		   data=(u8*)transfer[0].rx_buf;
		   data=data+1;
		   memcpy(response, data, len);
		   //memcpy(response, transfer[0].rx_buf + 1, len);
           //memcpy(response, transfer[0].rx_buf, len); 
	free(buffer);
//#endif
...
/*
    ...
    //l_rx_buf=transfer[0].rx_buf; 
    //буфер должен быть длинной + команды

    // memcpy(data, l_rx_buf +cmd_len, data_len+cmd_len);
	//memcpy(data, transfer[0].rx_buf +cmd_len, data_len);   
    //memcpy(data, transfer[0].rx_buf , data_len);
    // memcpy(l_data, transfer[0].rx_buf , data_len*sizeof(u32));
