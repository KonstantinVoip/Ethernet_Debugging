...
// надо посмотреть потом почему так !!!
//#ifdef __GNUC__
static RTXBD rtx __attribute__ ((aligned(8)));
//#else
//#error "rtx must be 64-bit aligned"//потом исправлю директиву
//#endif
...
uint tsec_local_mdio_read(volatile tsec_mdio_t *phyregs, uint phyid, uint regnum)
{
	 ...
	 * number into MIIMADD */
	 //phyregs->miimadd = (phyid << 8) | regnum;
      phyregs->miimadd = (phyid << 8) | regnum;
      printf("STOP\n"); 
     // phyregs->miimadd = 0x00000000+c;
      //phyregs->miimadd = 0x00000030;
       //phyregs->miimadd = 0x00000000;  //MII Control_Register
       //phyregs->miimadd=  0x00000001;  //MII Status Register
        // phyregs->miimadd=  0x00000020;
       // phyregs->miimadd=  0x00000004;
        
	/* Clear the command register, and wait */
	phyregs->miimcom = 0;
	//нужно иметь ввиду что asm("msync") ->> эквивалент интструкции asm("msync");
...
//Добавляю свою структуру для DP83848
