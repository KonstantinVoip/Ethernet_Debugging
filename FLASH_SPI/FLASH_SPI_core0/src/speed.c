...
#define MPC85xx_GUTC  GPIO_ADDR+CONFIG_SYS_MPC85xx_GUTS_OFFSET //Это правильно
...
/*
	...
	//sysInfo->freqSystemBus = plat_ratio * CONFIG_SYS_CLK_FREQ;
      sysInfo->freqSystemBus = plat_ratio * get_board_sys_clk(0);


	// Divide before multiply to avoid integer
	// overflow for processor speeds above 2GHz 
	half_freqSystemBus = sysInfo->freqSystemBus/2;
	for (i = 0; i < 1/*cpu_numcores()*/; i++) {
   ...
   */
  ...
  #else
	{
	 ...
	 //Нужно доделать Ассемблерное получение значения lcrr_div=1;
	 //сейчас пока стоит заглушка!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
int do_clocks()
{
	...
	printf("System Bus:%-4s MHz\n", strmhz(buf, gd->bus_clk));
	printf("Etsec:%-4s MHz\n", strmhz(buf, gd->bus_clk / 2));
...
#if defined(CONFIG_CPM2)
	printf("CPM:%-4s MHz\n", strmhz(buf, gd->cpm_clk));
	printf("SCC:%-4s MHz\n", strmhz(buf, gd->scc_clk));
	printf("BRG:%-4s MHz\n", strmhz(buf, gd->brg_clk));
#endif
