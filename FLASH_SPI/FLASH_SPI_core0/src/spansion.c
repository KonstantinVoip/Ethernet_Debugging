...
static inline struct spansion_spi_flash *to_spansion_spi_flash(struct spi_flash*flash)
{
	...
	//return spansion_spi_flash_params
	 
}

static const struct spansion_spi_flash_params spansion_spi_flash_table[] = {
	{SPSN_ID_S25FL008A,0,256,256,16,"S25FL008A"},		
	{SPSN_ID_S25FL016A,0,256,256,32,"S25FL016A"},                
	{SPSN_ID_S25FL032A,0,256,256,64,"S25FL032A"},
	{SPSN_ID_S25FL064A,0,256,256,128,"S25FL064A"},
	{SPSN_ID_S25FL128P,SPSN_EXT_ID_S25FL128P_64KB,256,256,256,"S25FL128P_64K"},
	{SPSN_ID_S25FL128P,SPSN_EXT_ID_S25FL128P_256KB,256,1024,64,"S25FL128P_256K"},
...
static int spansion_read_fast_chunks(struct spi_flash *flash,u32 offset, size_t data_len, void *buf)
{
	...
	while(num_chunks --) {
		...
		//debug("READ: 0x%x => cmd = { 0x%02x 0x%02x%02x%02x%02x } len = 0x%x\n",offset, cmd[0], cmd[1], cmd[2], cmd[3], cmd[4], len);
          printf("READ: 0x%x => cmd = { 0x%02x 0x%02x%02x%02x%02x } len = 0x%x\n",offset, cmd[0], cmd[1], cmd[2], cmd[3], cmd[4], len);  
		...
		//l_buf=buf;
		//l_buf+=max_tran_len;
		//buf += max_tran_len;                                      //zavtra ispravit
...
static int spansion_write(struct spi_flash *flash,u32 offset, size_t len, const void *buf)
{
	...
	for (actual = 0; actual < len; actual += chunk_len) 
	{
		...
		ret = spi_flash_cmd_write(flash->spi, cmd, 4,my_buf + actual, chunk_len);   //zavtra ispravit
		if (ret < 0) {
...
//unsigned long sector_size;
	...
	int ret;
	u8 cmd[4];
//Ставлю пока заглушку потом организую общую структуру!!! чтобы получать из
//неё данные
unsigned long sector_size;
unsigned long page_size;
...
struct spi_flash *spi_flash_probe_spansion(struct spi_slave *spi, u8 *idcode)
{
	...
	spsn->flash.size = params->page_size * params->pages_per_sector * params->nr_sectors;
